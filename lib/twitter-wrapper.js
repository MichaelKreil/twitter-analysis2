		...
		if (!bigResult) {
			...
			Object.keys(result).forEach(key => {
				switch (key) {
					...
					break;
					case 'users':
						bigResult.users = bigResult.users.concat(result.users);
					break;
					case 'next_cursor':
...
function twitterFetch(type, params, cbFetch) {
	accounts.get(type, (account, freeAccount) => {
		account.client.get(type, params, (error, result, response) => {
			...
			if (error && response && (response.statusCode === 429)) throw Error('429 Too Many Requests')
			...
			if (error) {
				...
				if (result.error === 'Not authorized.') {
					console.error(colors.yellow('Not authorized: Private account?'))
					finish();
...
function Accounts() {
	...
	var requestTypes = [
		...
		{key:'application/rate_limit_status', rateLimit:  180 },
		{key:'favorites/list',                rateLimit:   75 },
		{key:'followers/ids',                 rateLimit:   15 },
		{key:'followers/list',                rateLimit:   15 },
		{key:'friends/ids',                   rateLimit:   15 },
		{key:'friends/list',                  rateLimit:   15 },
		{key:'friendships/show',              rateLimit:   15 },
		{key:'help/configuration',            rateLimit:   15 },
		{key:'help/languages',                rateLimit:   15 },
		{key:'help/privacy',                  rateLimit:   15 },
		{key:'help/tos',                      rateLimit:   15 },
		{key:'lists/list',                    rateLimit:   15 },
		{key:'lists/members',                 rateLimit:   75 },
		{key:'lists/members/show',            rateLimit:   15 },
		{key:'lists/memberships',             rateLimit:   75 },
		{key:'lists/ownerships',              rateLimit:   15 },
		{key:'lists/show',                    rateLimit:   75 },
		{key:'lists/statuses',                rateLimit:  900 },
		{key:'lists/subscribers',             rateLimit:   15 },
		{key:'lists/subscribers/show',        rateLimit:   15 },
		{key:'lists/subscriptions',           rateLimit:   15 },
		...
		{key:'trends/available',              rateLimit:   75 },
		{key:'trends/closest',                rateLimit:   75 },
		{key:'trends/place',                  rateLimit:   75 },
		{key:'users/lookup',                  rateLimit:  300 },
		{key:'users/show',                    rateLimit:  900 },
		{key:'users/suggestions',             rateLimit:   15 },
	];
	...
	me.get = function (type, cb) {
		...
		if (list.length <= 0) throw Error('Unknown requestTypes');
	...
	function Account(name, key, secret) {
		...
		var me = {
			...
			setReset: function (v) { reset = v; if (v > Date.now()/1000 + 15*60 + 2) throw Error(v+' vs '+Date.now()/1000); },
